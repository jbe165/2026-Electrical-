...
while(current_lap <= NUM_LAPS and track.is_driving()):
    ...
    if not track.is_driving() and current_lap < NUM_LAPS:
        print(f"Lap {current_lap} - Time: {sim_time:.2f}s, SoC: {accumulator.get_soc_percent():.1f}%, "
              f"V: {accumulator.pack_voltage:.1f}V")
        current_lap += 1
        track = Track(defined_tracks.calder_autox, vehicle)

# Results
print("\n=== Simulation Results ===")
print(f"Total Time: {sim_time:.2f}s ({sim_time/60:.2f} minutes)")
print(f"Laps Completed: {current_lap}")
